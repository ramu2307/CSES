...
public class CSES_Dynamic_Range_Minimum_Queries {
    ...
    public static void update(int idx, int low, int high, int i, long val, long[] segment) {
        ...
        int mid = (low + high)/2;
        if(i <= mid) {
            update(2 * idx + 1, low, mid, i, val, segment);
        }
        else {
            update(2 * idx + 2, mid + 1, high, i, val, segment);
        }
